...
void tty_init(void) {
    ...
    USART1->CR1 &= ~(1 << 28 | 1 << 12); // word size
...
void setup_gpio() {
    ...
    GPIOA ->MODER |= (GPIO_MODER_MODER0 | GPIO_MODER_MODER1 | GPIO_MODER_MODER2 | GPIO_MODER_MODER4);
...
static void action(char **words) {
    ...
    if (words[0] != 0) {
        ...
        if (strcasecmp(words[0],"gen") == 0) {
              ...
              int freq = atoi(words[1]);

              step = freq * N / RATE * (1<<16);
