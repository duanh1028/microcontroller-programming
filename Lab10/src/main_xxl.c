...
void tty_init(void) {
    ...
    GPIOA->AFR[1] |= 1<<4;//high AF1
    GPIOA->AFR[1] |= 1<<8;
    RCC->APB2ENR |= RCC_APB2ENR_USART1EN;

    USART1->CR1 &= ~USART_CR1_UE;
    USART1->CR1 &= ~(1<<12);// word size
    USART1->CR1 &= ~(1<<28);
...
static void action(char **words) {
    if (words[0] != 0) {
        ...
        if (strcasecmp(words[0],"gen") == 0) {
            ...
            step = gen * N / 100000.0 * (1 << 16);
            printf("finished, check waveform.\n");
            return;
        }
        if (strcasecmp(words[0],"say") == 0) {
            printf("%s\n", words[1]);
            return;
        }
