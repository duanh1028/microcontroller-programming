...
void USART1_IRQHandler(void) {
	...
	if((USART1->ISR & USART_ISR_RXNE)==1)
				 insert_echo_char(USART1->RDR);
			if((USART1->ISR & USART_ISR_TXE)==1){
				if (fifo_empty(&output_fifo)){
					USART1->CR1 &= ~USART_CR1_TXEIE;
				}
				else
					fifo_remove(&output_fifo);
...
void setup_timer6() {
	...
	RCC->APB1ENR |= RCC_APB1ENR_TIM6EN;
		    	...
		    	TIM6->CR1 |= TIM_CR1_CEN; // or TIM6->CR1 &= ~TIM_CR6_CEN;
		    	NVIC->ISER[0] = 1<<TIM6_DAC_IRQn;//start timer
}
...
void init_wavetable(void)
{
    // Student code goes here...
	for(int a = 0;a <N;a++){
			wavetable[a] = sin(2*M_PI*a/N)*32767;
		}
